...
const Discover = () => {
  ...
  const playonlinedata = Data.filter((a)=>{
    return a.typesl == "playonline"
  })
  const settings = {
    infinite: false,
    speed: 500,
  ...
  return (
    ...
    <div className="card-body text-light  p-0 mt-2">
    <a target="_blank" href={item.toUrl}>
      <p className="card-title ">{item.title}</p></a>
      <Link  className={item.class}>{item.btn}</Link>
      <div className="d-flex dd mt-2">
 <button href="{playonlinedata.toUrl}" className={item.discountbuttonclass}>{item.dicountrate}</button>
     <p className='ms-3 lt'>{item.price}</p>
     <p className='ms-3'>${item.discountedprice}</p>
          ...
          <div className="update-games">
            <div className="card col-lg-4 border-0 p-0">
              ...
              <div className="card-body">
                ...
                <p className="mt-2 price">$99.99</p>
              </div>
            </div>
